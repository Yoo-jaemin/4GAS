...
case ATCVER: len += _strcpy(&writeBuffer[len], "UA58-KFG-U""_""test"); break;
case ATCUV:
{
...
case ATCQ:
{
for (uint8_t chNum = 0; chNum < 3; chNum++) {
if (g_sens_measure.isGasSensor_Success[chNum] && g_nvm.flash.CalibrationGas[chNum] != 0.0f && g_led_status.isBlink == 0) {
if (chNum == 0 ||chNum == 2)
...
if (g_led_status.isBlink == 0) len += _itoa(Last_value.co2_gas, &writeBuffer[len]);
else len += _strcpy(&writeBuffer[len], "----");
} break;
case ATCH:
{
